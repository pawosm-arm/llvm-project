...
LegalizerHelper::lowerUADDSAT_USUBSAT(MachineInstr &MI) {
  ...
  Register LHS = MI.getOperand(1).getReg();
  Register RHS = MI.getOperand(2).getReg();
  const bool IsAdd = MI.getOpcode() == TargetOpcode::G_UADDSAT;

  LLT Ty = MRI.getType(Dst);
  unsigned TySize = Ty.getScalarSizeInBits();
  auto Sat = MIRBuilder.buildConstant(Ty, IsAdd ? APInt::getMaxValue(TySize)
                                                : APInt::getMinValue(TySize));
  auto Res = MIRBuilder.buildInstr(IsAdd ? TargetOpcode::G_UADDO
                                         : TargetOpcode::G_USUBO,
                                   {Ty, LLT::scalar(1)}, {LHS, RHS});
  MIRBuilder.buildSelect(Dst, Res.getReg(1), Sat, Res);
  MI.eraseFromParent();
  return Legalized;
}
...
LegalizerHelper::lowerSADDSAT_SSUBSAT(MachineInstr &MI) {
  ...
  Register LHS = MI.getOperand(1).getReg();
  Register RHS = MI.getOperand(2).getReg();
  const bool IsAdd = MI.getOpcode() == TargetOpcode::G_SADDSAT;

  LLT Ty = MRI.getType(Dst);
  unsigned TySize = Ty.getScalarSizeInBits();
  auto Zero = MIRBuilder.buildConstant(Ty, APInt::getMinValue(TySize));
  auto Min = MIRBuilder.buildConstant(Ty, APInt::getSignedMinValue(TySize));
  auto Max = MIRBuilder.buildConstant(Ty, APInt::getSignedMaxValue(TySize));
  auto Res = MIRBuilder.buildInstr(IsAdd ? TargetOpcode::G_SADDO
                                         : TargetOpcode::G_SSUBO,
                                   {Ty, LLT::scalar(1)}, {LHS, RHS});
  auto Neg = MIRBuilder.buildICmp(CmpInst::ICMP_SLT, LLT::scalar(1), Res, Zero);
  auto Sat = MIRBuilder.buildSelect(Ty, Neg, Max, Min);
  MIRBuilder.buildSelect(Dst, Res.getReg(1), Sat, Res);
  MI.eraseFromParent();
  return Legalized;
}
